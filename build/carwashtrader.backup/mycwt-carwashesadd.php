...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] )) {
	...
	if (!empty ($_POST['addpost_description'])) $description = htmlentities($_POST['addpost_description']); 
		else $error[] = 'Please enter a brief description of the car wash';
	if (!empty ($_POST['addpost_address'])) $address =  htmlentities($_POST['addpost_address']); 
	...
	else if (!empty ($_POST['addpost_postcode'])) $postcode =  htmlentities($_POST['addpost_postcode']); 
	...
	$prices = htmlentities($_POST['addpost_prices']);
	$openinghours = htmlentities($_POST['addpost_openinghours']);
	$contactnumber = htmlentities($_POST['addpost_contactnumber']);
	$website = htmlentities($_POST['addpost_website']);
	...
	if($_POST['action'] == 'new-post') {
		...
		if (sizeof($error) == 0) {
			...
			//set geocode for map
			$geocode = '';
			$resp = wp_remote_get( "http://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($postcode)."&sensor=false" );
