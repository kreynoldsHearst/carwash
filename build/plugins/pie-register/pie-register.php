...
var ajaxurl = '<?php echo admin_url('admin-ajax.php'); ?>';
	...
	function pie_save_registration()
	{
		...
		if(sizeof($errors->errors) == 0)
		{
			...
			//Inserting User
			$pass = $_POST['password'];
			//Kevin added - parse user account choice
			$userRole = $_POST["radio_8"];
			//End Kevin added
			$user_data = array(
							   'user_pass' 		=> $pass,
							   'user_login' 	=> $_POST['username'],
							   'user_email' 	=> $_POST['e_mail'],
							   //Kevin added - allow for choosing account type
			...
			*/
			...
			//Kevin added = allow for user selecting account type
			$new_role = $userRole[0];
			//End Kevin added
			...
			else if($confirmation_type == "redirect")
			{
                ...
                <?php*/
				wp_redirect($fields['submit']['redirect_url'].'?user_id='.$user_id.'&hash='.$hash);
				exit;
			}
			else
			{
